...
(defun token->json (plist)
  (let ((h (alexandria:plist-hash-table plist))
    ...
    (alexandria:alist-hash-table
      `(("form" . ,(gethash :form h))
	("lemma" . ,(gethash :lemma h))
	("kind" . ,(if (consp kind)
		       (alexandria:alist-hash-table (list (cons (symbol->string (car kind))
								(cdr kind))))
		       (symbol->string kind)))
	...
	("meta" . ,(alexandria:alist-hash-table
		    (loop
		      ...
		      unless (member key '(:form :lemma :kind :senses :unsure :tag))
